...
namespace Sim::Testing
{
    ...
    {
        OrderFactoryTestFixture() { mOrderFactory = std::make_unique<OrderFactory>(); }
    ...
    TEST_F(OrderFactoryTestFixture, CreateOrder_ReturnsOrder)
    {
        ...
        auto orderPtr = mOrderFactory->createOrder(order, [](Order* order) { delete order; });
    ...
    TEST_F(OrderFactoryTestFixture, CreateOrder_DeleterCalled)
    {
        ...
        {
            auto orderPtr = mOrderFactory->createOrder(order, [&modified](Order* order) {
                modified = true;
                delete order;
            });
            ASSERT_EQ(orderPtr->mClientId, 1);
